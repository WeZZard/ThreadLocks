    ...
    def append_text(self, text, file, line):
        ...
        if self.line_directive:
            if (file, line) != self.last_file_line:
                ...
                # But if the new text contains any line breaks, we can
                # create
                # one
                elif '\n' in text:
                    i = text.find('\n')
...
'''
    ...
    source_io_wrapper_utf_8 = io.TextIOWrapper(
        args.file.buffer, encoding='utf-8'
    )
    source_content = source_io_wrapper_utf_8.read()

    ast = parse_template(args.file.name, source_content)
    if args.dump:
        print(ast)
    # Allow the template to import .py files from its own directory
    sys.path = [os.path.split(args.file.name)[0] or '.'] + sys.path

    target_io_wrapper_utf_8 = io.TextIOWrapper(
        args.target.buffer, encoding='utf-8'
    )

    target_io_wrapper_utf_8.write(
